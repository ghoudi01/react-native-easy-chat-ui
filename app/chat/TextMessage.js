...
export default class TextMessage extends PureComponent {
  render () {
    const { isTyping,isSelf, message, messageErrorIcon, views, isOpen, rightMessageBackground, leftMessageBackground, reSendMessage, chatType, isReadStyle, showIsRead, ImageComponent } = this.props
    return (
      <View
      ...
      >
        ...
        >
          <View style={[styles.container, { backgroundColor: isSelf ? rightMessageBackground
              : leftMessageBackground,width:isTyping?70:null,height:isTyping?40:null }]}>
            {isTyping!=true?views: <TypingAnimation 
        dotColor={isSelf?"#FFF":"rgb(0,38,58)"}
        dotMargin={9}
        dotAmplitude={3}
        dotSpeed={0.15}
        dotRadius={3.5}
